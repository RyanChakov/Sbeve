...
public class CaveDestroyer : MonoBehaviour
{
    ...
    private void Update()
    {
        if (transform.parent.gameObject.transform.position.x < 800)
        {
            if (timer==1)
            {
                TryEditTerrain1();
    ...
    private void RaycastToTerrain1(bool addTerrain)
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, maxReachDistance))
        {
            ...
            {
                EditTerrain1(new Vector3(hitPoint.x +Random.Range(-15, 20), hitPoint.y + Random.Range(-15, 20), hitPoint.z + Random.Range(-15, 20)), addTerrain, force, range);
            }
